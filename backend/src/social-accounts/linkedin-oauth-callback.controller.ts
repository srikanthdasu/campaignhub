...
// Same shape as MetaOAuthCallbackController — LinkedIn's server redirects the user's browser back
...
@Controller('social-accounts/linkedin')
export class LinkedInOAuthCallbackController {
  ...
  async callback(
    ...
    if (error || !code || !state) {
      const message = errorDescription || error || 'LinkedIn login was cancelled or failed';
      return res.redirect(`${returnTo}?connect_error=${encodeURIComponent(message)}`);
    }

    try {
      const { clientId, actorId } = this.linkedInOAuth.decodeState(state);
      const accessToken = await this.linkedInOAuth.exchangeCodeForToken(code);
      const profile = await this.linkedInOAuth.fetchProfile(accessToken);
